...
CREATE TABLE if not exists assets (
  tag varchar(7) NOT NULL DEFAULT '' primary key,
...
CREATE TABLE if not exists sold_via (
  sold_via_id int NOT NULL DEFAULT '0',
  name varchar(255) DEFAULT NULL,
...
AS received,asset_types.name 
AS asset_type,assets.manufacturer 
AS manufacturer,left(assets.product,10) 
AS product,left(assets.model,10) AS model,locations.name 
AS location from (
